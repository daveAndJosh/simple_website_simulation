 ...
 * modified by David Haighton to use bigports. This is not my work but I need it to get my work done
 *
...
#include <cadmium/core/exception.hpp>
...
namespace test {
     ...
     */
    template<typename MSG>
    struct BigIEStreamState {
        BigEventParser<MSG> parser;  //!< Input events parser.
         ...
         */
        explicit BigIEStreamState(const char* filePath): parser(BigEventParser<MSG>(filePath)), lastInputRead(), clock(), sigma() {
     ...
     */
    template<typename MSG>
    std::ostream& operator<<(std::ostream &out, const BigIEStreamState<MSG>& state) {
        out << state.sigma;
        return out;
    }
     ...
     */
    template<typename MSG>
    class BigIEStream : public Atomic<BigIEStreamState<MSG>> {
     public:
        BigPort<MSG> out;
         ...
         */
        BigIEStream(const std::string& id, const char* filePath): Atomic<BigIEStreamState<MSG>>(id, BigIEStreamState<MSG>(filePath)) {
            out = Atomic<BigIEStreamState<MSG>>::template addOutBigPort<MSG>("out");
        }
         ...
         */
        void internalTransition(BigIEStreamState<MSG>& state) const override {
         ...
         */
        void externalTransition(BigIEStreamState<MSG>& state, double e) const override {
         ...
         * @param state reference to the current model state.
         */
        void output(const BigIEStreamState<MSG>& state) const override {
         ...
         */
        [[nodiscard]] double timeAdvance(const BigIEStreamState<MSG>& state) const override {
            return state.sigma;
        }
    };
}
